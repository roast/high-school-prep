...
1. 暑期初升高空白期不知学什么、怎么安排——按人教版章节给到二级知识点的完整目录与进度表
2. 报班节奏不跟随孩子、听不懂也来不及问——随时提问、随时降速、随时换讲法
3. AI 工具零散玩、不会用于系统学习——把 AI 锁进特级教师角色，按教学流程驱动
4. 抽象知识看文字难懂——函数图像、物理运动、化学反应都能可视化
5. 出题靠老师、变式练习稀缺——按知识点+难度一键生成可打印 PDF
6. 错题要手动抄、复盘找不到原题——答错自动入错题本，复盘时按错点重出变体题
7. 学完不知掌握没——每概念后递进自检 + 状态自动更新（未学/学习中/已掌握）
8. 章节学完没整体梳理——章节末自动出 Mermaid 思维导图

**提供的功能：**

1. 5 科全覆盖（语文/数学/物理/化学/英语，人教版高一上学期）
...
9. 进度跟踪：每科 progress.md 知识点状态表 + 当日总结
10. 思维导图：章节末 Mermaid 知识结构图
...
```
给物理 ch1「加速度」做一个 HTML 互动演示页，落
physics/knowledge/ch1-加速度/demo.html，生成后在浏览器打开。
分四块从易到难：

1. 入门：滑块调 v0 和 a，实时画 v-t 图，配一句话提示
   "a>0 加速 / a<0 减速 / a=0 匀速"。
2. 进阶：同参数下再画 x-t 图，附引导问题"斜率代表什么"。
3. 挑战：加第二辆车对比，动画演示谁追上谁，带暂停和倍速。
4. 拓展：v-t 图上能框选区间，显示面积=位移。

vanilla JS + KaTeX 渲染公式，控件加 aria-label。每块末尾留一句
引导问题不给答案。
```
