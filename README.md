...
### 场景 1：生成加速度互动演示页（从易到难全覆盖）

```
为物理 ch1「1.4 速度变化快慢的描述——加速度」生成一个单文件 HTML
互动演示页，落 physics/knowledge/ch1-加速度/demo.html，生成后在
默认浏览器打开。要求覆盖从易到难四个环节，全部在一个页面里用
分节切换：

1. 入门——直觉建立
   两个滑块调初速度 v0（-20~20 m/s）和加速度 a（-10~10 m/s²），
   实时画 v-t 图像（直线），标注斜率=a。下方用一句话文字反馈：
   "a>0 速度在增""a<0 速度在减""a=0 匀速"，让初学者看图也看字。

2. 进阶——位移理解
   同一参数下再画 x-t 图像（抛物线），标注顶点。引导问题文本：
   "v-t 图直线斜率代表什么？x-t 图切线斜率又代表什么？"

3. 挑战——双车对比
   加第二个物体（v0_2、a_2 滑块），canvas 动画演示两车同向运动，
   实时显示两车位置/速度。提供"开始/暂停/重置"按钮和倍速选择
   (0.5x/1x/2x)。当两车相遇时高亮提示。引导学生发现：
   "速度大的不一定跑得远，加速度决定谁追上谁。"

4. 拓展——面积即位移
   在 v-t 图上可拖动鼠标框选一段时间区间，高亮显示该区间内
   直线下方的面积，旁边数字显示面积值（=位移）。让学生自己
   归纳"v-t 图面积=位移"。

要求：vanilla JS + KaTeX CDN 渲染公式（$v=v_0+at$、$x=v_0t+\frac12 at^2$），
所有交互控件加 aria-label，无构建步骤。每个环节末尾附一句引导问题，
不直接给答案。生成后告诉我文件路径并打开浏览器。
```
